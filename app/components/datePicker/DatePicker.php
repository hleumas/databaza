...
class DatePicker extends Forms\Controls\BaseControl
{
	...
	public function getControl()
	{
		...
		if ($min !== NULL) $control->data['min'] = $min->format($this->format);
		if ($max !== NULL) $control->data['max'] = $max->format($this->format);
		if ($this->value) $control->value = $this->value->format($this->format);
		return $control;
	}
	...
	public function setValue($value)
	{
		...
		if (!isset($rawValue) && isset($value)) {
			$rawValue = $value->format($this->format);
		}
